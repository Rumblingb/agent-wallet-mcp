...
const PIP_PACKAGES = ["mcp>=1.0.0", "uuid>=1.30"];

function log(msg) {
  process.stderr.write(`[agent-wallet-mcp] ${msg}\n`);
}
...
function installDeps() {
  ...
  try {
    runSync("python3 -c 'import mcp; import uuid'", {
      stdio: ["inherit", "pipe", "pipe"],
    });
  ...
  try {
    ...
    log("Try running: pip install mcp uuid");
    process.exit(1);
  }
}
